...
router.post('/', async (req, res) => {
  try {
    ...
    const [rows] = await pool.query('SELECT c.*, p.name AS headTeacher FROM classes c LEFT JOIN professors p ON c.head_teacher_id = p.id WHERE c.id = ?', [classId]);
...
router.put('/:id', async (req, res) => {
  try {
    ...
    await pool.query(
      `UPDATE classes SET name=?, specialty=?, level=?, student_count=?, head_teacher_id=?, representative=?, schedule_progress=?
       WHERE id=?`,
      [name, specialty, level, student_count, head_teacher_id || null, representative, schedule_progress, req.params.id]
    );
    const [rows] = await pool.query('SELECT c.*, p.name AS headTeacher FROM classes c LEFT JOIN professors p ON c.head_teacher_id = p.id WHERE c.id = ?', [req.params.id]);
    res.json(rows[0]);
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: 'Erreur serveur.' });
